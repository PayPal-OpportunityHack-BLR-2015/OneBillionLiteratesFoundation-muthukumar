 ...
 * @since 1.0
 */
public class JPollingUserHelper {

	/**
	 * @param userTO
	 */
	public JPollingUserHelper(UserTO userTO) {
	}
	...
	public String multipleUser(HttpServletRequest request) {
		...
		try {
			if (request.getParameter("common_user_name") != null
				...
				startRange=Integer.parseInt(request.getParameter("start_value"));
				endRange=Integer.parseInt(request.getParameter("end_value"));
				mailId=request.getParameter("common_mail_id");
				int count=usersDAO.saveMultipleUser(userId, startRange, endRange, password,mailId);
				System.out.println(count+" Number of Users Saved successfully");
				System.out.println("Rediecting to Load User Page");
				xml = xml.append(loadUsers(request));
			}
			else
	...
	public String loadHome(HttpServletRequest request){
		...
		xml.append("<td width='62%' ><div align='left'>Total Users Count</div></td>");
		xml.append("<td width='38%' ><div align='left'>"+ userCount +"</div></td>");
		xml.append("</tr>");
		xml.append("<tr>");
		long questionCount =questionsDAO.getAdminQuestionsCount(userTO);
		xml.append("<td > <div align='left'>Total Question Count </div></td>");
		xml.append("<td ><div align='left'>"+ questionCount +"</div></td>");
		xml.append("</tr>");
		xml.append("<tr >");
		long answerCount =answersDAO.getAnswerCount();
		xml.append("<td > <div align='left'>Total Answer Count </div></td>");
		...
		xml.append("<tr>");
		xml.append("<td colspan='2' class='data_table_td_1'>" + questionsDAO.getLastQuestion().getQuestion() + "</td>");
		xml.append("</tr>");
		xml.append("<tr>");
		xml.append("<td>&nbsp;</td>");
		xml.append("<td><table width='100%' class='data_table'>");

		List<AnswerTO> ansList = answersDAO.getAnswerList(questionsDAO.getLastQuestion().getId());
		int i = 0;
	...
	public String loadCreateQuestion(HttpServletRequest request){
		...
		xml.append("<div class='headings'>Create Question - Answers </div>");
		xml.append("<table width='100%' class='data_table'>");
		xml.append("<tbody><tr>");
		xml.append("<td width='' height='40' class='data_table_td_1'>Question : </td>");
		xml.append("<td width='80%' ><textarea id='question' row='3' column='253'style='width: 549px; height: 73px;'></textarea></td>");
		xml.append("</tr>");
		xml.append("<tr>");
		xml.append("<td class='data_table_td_1'>End Date : </td>");
		xml.append("<td valign='top' ><input type='text' id='end_date' class='fields' value='"+JPollingUtil.getDateTime()+"'></td>");
		xml.append("</tr>");
		xml.append("<tr>");
		xml.append("<td class='data_table_td_1'>Answers : </td>");
		xml.append("<td valign='top' ></td>");
		xml.append("</tr>");
		xml.append("<tr>");
		xml.append("<td class='data_table_td_1'>Ansewer 1 :</td>");
		xml.append("<td ><textarea id='answer_1' style='width: 443px; height: 26px;'></textarea></td>");
		xml.append("</tr>");
		xml.append("<tr>");
		xml.append("<td class='data_table_td_1'>Ansewer 2 :</td>");
		xml.append("<td ><textarea id='answer_2' style='width: 443px; height: 26px;'></textarea></td>");
		xml.append("</tr>");
		xml.append("<tr>");
		xml.append("<td class='data_table_td_1'>Ansewer 3 :</td>");
		xml.append("<td ><textarea id='answer_3' style='width: 443px; height: 26px;'></textarea></td>");
		xml.append("</tr>");
		xml.append("<tr>");
		xml.append("<td class='data_table_td_1'>Ansewer 4 :</td>");
		xml.append("<td ><textarea id='answer_4' style='width: 443px; height: 26px;'></textarea></td>");
		xml.append("</tr>");
		xml.append("<tr>");
		xml.append("<td></td>");
	...
	public String loadUserList(HttpServletRequest request){
		...
		try{
			...
			boolean pickUser =  Boolean.parseBoolean(request.getParameter("pickUesr"));
			
			UsersDAO usersDAO = new UsersDAO();
			long count = usersDAO.getUsercount(null);
			List<UserTO> usersList = new ArrayList<UserTO>();
			//usersList = usersDAO.getUsers(start);
			
			xml.append("<table width='100%' class='data_table'>");
			xml.append("<tbody><tr>");
			xml.append("<td width='3%' class='data_table_td_head'>&nbsp;</td>");
			xml.append("<td width='25%' class='data_table_td_head'>User ID</td>");
			//xml.append("<td width='14%' class='data_table_td_head'>Password</td>");
			xml.append("<td width='25%' class='data_table_td_head'>Mail ID</td>");
			xml.append("<td width='25%' class='data_table_td_head'>Voter ID</td>");
			xml.append("<td width='25%' class='data_table_td_head'>Verified</td>");
			xml.append("</tr>");
			xml.append("<tr>");
			for (UserTO userTO : usersList) {
				...
				if(pickUser){
					xml.append("<input type='checkbox' id='"+ userTO.getUserId() + "' onChange='selecteOrUnselectUser("+ userTO.getUserId() + ");'>");
				}
				xml.append("</td>");
				xml.append("<td width='25%' >" + userTO.getUserName() + "</td>");
				//xml.append("<td width='14%' >" + userTO.getPassword() + "</td>");
				xml.append("<td width='25%' >" + userTO.getMailId() + "</td>");
			...
			xml.append(getPageNavigation(count,start,"navigateUserList","\""+pickUser+"\""));
			xml.append("</div>");
		}catch (Exception e) {
	...
	private String getPageNavigation(long count, long start, String onClick, String extraParem) {
		...
		if(currentPage>1){
			...
			if(extraParem!=null){
				previousOnClick= onClick +"("+(start-perPage)+","+extraParem+");";
			}else{
				previousOnClick= onClick +"("+(start-perPage)+");";
			}
			html.append("<a href='#' onclick='").append(previousOnClick).append("'><img width='25' height='25' src='/JPollingWeb/StaticContent/Images/PreviousButton_blue.png'></a>");
		} else{
		...
		html.append(currentPage).append("&nbsp;/&nbsp;").append(totalPage).append("</div></td>");
		html.append("<td width='25%'>");
		if(currentPage<totalPage){
			...
			html.append("<a href='#' onclick='").append(nextOnClick).append("'><img width='25' height='25' src='/JPollingWeb/StaticContent/Images/NextButton_blue.png'></a>");
		}else{
	...
	public String loadCreateUsers(HttpServletRequest request) {
		...
		xml.append("<td width='31%'><table align='right'>");
		xml.append("<tbody><tr>");
		xml.append("<td class='quick_links'><a href='#' onclick='loadCreateUsers(\""+"single"+"\")'><img width='25' height='25' title='Create Single User' src='/JPollingWeb/StaticContent/Images/add_user.png'/></a></td>");
		...
		xml.append("<td>&nbsp;</td>");
		xml.append("<td>&nbsp;</td>");
		xml.append("<td><input type='button'  onclick='createUsers(\""+type+"\")' value='Create User' id='createUsers'></td>");
		xml.append("</tr>");
		xml.append("</tbody></table>");
		xml.append("</div>");
		xml.append("<p>&nbsp;</p>");
	...
	public String createQuestionPickUser(HttpServletRequest request){
		...
		try{
			...
			questionTO.setStartDate(JPollingUtil.getSqlDateTime());
			questionTO.setEndDate(JPollingUtil.getSqlDateTime());
			questionTO=questionsDAO.saveQuestion(questionTO);
			
			AnswersDAO answerDAO;
			AnswerTO answerTO;
			StringTokenizer answers=new StringTokenizer((String)request.getParameter("answers"),"!_");
		...
		xml.append("<table>");
		xml.append("<tbody><tr>");
		xml.append("<td align='right'><input type='button' class='submit_button' value='Assign Question' onclick='assignQuestion("+questionTO.getId()+")'></td>");
	...
	public String loadQuestion(HttpServletRequest request) {
		...
		try {
			...
			if (userTO.getUserType().equalsIgnoreCase("admin")) {
				...
				xml.append("<td width='63%'><h4 align='left' class='quick_links'> Question - Answers</h4></td>");
				xml.append("<td width='31%'><table align='right'>");
				xml.append("<tbody><tr>");
				xml.append("<td class='quick_links'><a onclick='loadCreateQuestion()' href='#'><img width='25' height='25' alt='X' src='/JPollingWeb/StaticContent/Images/add.png'></a></td>");
				xml.append("</tr>");
				xml.append("</tbody></table></td>");
				xml.append("</tr>");
				xml.append("</tbody></table>");
	...
	public String loadQuestionList(HttpServletRequest request){
		...
		try {
			...
			//isReport = Boolean.parseBoolean(request.getParameter("isReport"));
	...
	public String generateQuestionList(UserTO userTO, int start, boolean isReport){
		StringBuffer xml = new StringBuffer();
		try {
			...
			if (tot > 0) {
				...
				while (itrList.hasNext()) {
					...
					xml.append("<tr>");
					if(isReport){
						xml.append("<td class='data_table_td_1' width='5%' height='37'><a href='' onclick='loadSubReport("+question.getId()+")'><img width='25' height='25' alt='Report' src='/JPollingWeb/StaticContent/Images/pie_chart.png'/></a></td>");
						...
						xml.append("<td class='data_table_td_1'>");
						xml.append("<a style='cursor:pointer;' onclick='showAnswer(" + question.getId() + "," + tot + ");'>");
						xml.append(question.getQuestion());
						xml.append("</a>");
						xml.append("<span><a href='#' img='/JPollingWeb/StaticContent/Images/NextButton1.png' onclick='deleteQuestion("+question.getId()+")'>del</a></span></td>");
					}
					xml.append("<td class='data_table_td_1'>");
			...
			xml.append("<div class='data_table_footer' id='page_navigation'>");
			if(isReport){
				xml.append(getPageNavigation(count, start, "navigateQuestionList", "true"));
			}else{
				xml.append(getPageNavigation(count, start, "navigateQuestionList", "false"));
			}
			xml.append("</div>");
		} catch (Exception e) {
	...
	public String showAnswer(HttpServletRequest request) {
		...
		AnswersDAO answersDAO = new AnswersDAO();
		try {
			long questionId = Integer.parseInt(request.getParameter("question_id"));
			...
			xml.append("<td width='92%' class='data_table_td_head'>Answers</td>");
			xml.append("</tr>");
			int i = 0;
			if (answerList.size() > 0) {
				...
				while (itr.hasNext()) {
					...
					if("user".equals(userTO.getUserType())){
						xml.append("<input id='"+answerTO.getId()+"' name='answers' type='radio' onclick='postAnswer("+answerTO.getQuestion().getId()+","+answerTO.getId()+")'>");
					}else{
						xml.append(i);
					}
					xml.append("</td>");
	...
	public String loadClientHome(HttpServletRequest request) {
		...
		try {
			...
			xml.append("<td width='93%' valign='top'>"+userTO.getUserName()+"</td>");
	...
	public String assignQuestion(HttpServletRequest request){
		...
		StringTokenizer userIds=new StringTokenizer((String)request.getParameter("user_ids"),"!_");
	...
	public String postAnswer(HttpServletRequest request) {
		...
		if (userTO != null && userTO.getUserId() != 0) {
			if (request.getParameter("ans_id") != null && request.getParameter("q_id") != null) {
	...
	public String generateMailforUser(UserTO userTO){
		...
		mailContent.append("<h4> This is an auto generated mail from JLearning System</h4>");
		...
		mailContent.append("<a href='"+ JPollingLayout.URL +"'>Click here to Sign in</a>");
		mailContent.append("</body>");
		mailContent.append("</html>");
	...
	public String loadReport(HttpServletRequest request){
		...
		String workAreaContent = jReportingComponent.generateReport(questionId,start,userTO);
